...
public class ParameterRequestWrapper extends HttpServletRequestWrapper {
    ...
    ParameterRequestWrapper(HttpServletRequest request, Map<String, Object> extendParams) {
        this(request);
        //这里将扩展参数写入参数表
    ...
    private void addParameter(String name, Object value) {
        if (value != null) {
            if (value instanceof String[]) {
                ...
                String[] s = new String[((JSONArray) value).size()];
                for(int i=0; i < ((JSONArray) value).size(); i++) {
                    s[i] = ((JSONArray) value).get(i)+"";
                }
                params.put(name, s);
            } else {
